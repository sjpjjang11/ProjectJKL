...
public class csAIPathFinding : MonoBehaviour
{
	...
	private IEnumerator CoFindPath(Vector3 _startPosition, Vector3 _endPosition, csNode _exceptionNode)
	{
		...
		if (m_EndNode.NodeType != eNodeType.Obstacle)
		{
			...
			while (OpenList.Count > 0)
			{
				...
				for (int i = 0; i < /*Node.m_ListNeighbours.Count*/8; i++)
				{
					if(i == Node.m_ListNeighbours.Count)
					{
						break;
					}
		...
		{
			Debug.LogError("IsPathSuccess : " + IsPathSuccess + "  " + name + "  " + (m_EndNode.m_iOwner == gameObject.GetInstanceID()));
		}
